...
一个 Skill 是一个文件夹, 里面有个 `SKILL.md` Markdown 文件带 YAML frontmatter. [Anthropic 在 2025-10 发布这个 spec](https://www.anthropic.com/news/skills), 2025-12 开源; Claude Code, OpenAI Codex, Cursor, Gemini CLI, Hermes, Antigravity, Windsurf 等 50+ agents 都支持.

### 文件夹结构

```
skill-name/
├── SKILL.md          # 必选: name + description frontmatter + Markdown body
├── scripts/          # 可选: 辅助 shell / python 脚本
├── templates/        # 可选: 起始文件
...
Skills 分 3 层加载, 单个 agent 能 host 几百个 skills 不爆 context window:

| 时机 | 加载什么 | token 成本 |
|------|---------|------------|
| Session 启动 | 每个 skill 的 `name` + `description` (只 frontmatter) | ~100 tokens × N |
| Agent 判断 skill 相关 | 该 skill 的完整 `SKILL.md` body | ≤ 5000 tokens, 1 skill |
| Body 引用 `references/<file>.md` | 引用的那个 reference | 按需 |

### 怎么对比

| 方案 | Spec | Runtime | 跨 agent 可移植 |
|------|------|---------|-----------------|
| **SKILL.md** (本仓库) | 开放标准 | 无 (纯 Markdown) | ✅ 50+ agents |
| MCP server | 开放标准 | 需要跑 server | ✅ MCP clients |
| function calling | vendor-specific | 需要发代码 | ❌ 每个 agent 不一样 |
| system prompt stuffing | 无 | 无 | ❌ 10 个 skills 就爆 context |

### 本仓库

按这个 spec 写的 3 个 skill: [blocks](skills/agentic/blocks/SKILL.md) (tmux 多 agent 协调) · [dev-task](skills/productivity/dev-task/SKILL.md) (多子代理开发) · [session-summary](skills/productivity/session-summary/SKILL.md) (session 交接).

![blocks 2x2 — Manager + 4 workers in one tmux window](assets/blocks-2x2.svg)

## Features

- **跨 agent** — 同一份 SKILL.md 在 Hermes / Claude Code / Codex / Aider 都能跑 (profile flag / worktree / slash command 差异见 [compat 表](skills/agentic/blocks/references/agent-compatibility.md))
- **开放标准** — 严格遵守 [agentskills.io spec](https://agentskills.io/specification)
...
### 官方文档
- [agentskills.io spec](https://agentskills.io/specification) — 开放标准本身
- [Anthropic skills announcement](https://www.anthropic.com/news/skills) (2025-10) — 原始 writeup

### 社区
- [ComposioHQ/awesome-claude-skills](https://github.com/ComposioHQ/awesome-claude-skills) — 1000+ skills 精选列表
- [Vercel `npx skills` CLI](https://github.com/vercel-labs/skills) — 跨 agent 安装 (50+ agents)
- [SkillsMP.com](https://skillsmp.com) — 公开 GitHub SKILL.md auto-index

### Inspiration
- [Anthropic skills repo](https://github.com/anthropics/skills) — 示例 skills
- [Lenny's Newsletter](https://www.lennysnewsletter.com/p/everyone-should-be-using-claude-code) — 50 个 agent 编码 use cases
- [Notion Agent Skills](https://www.notion.so/notiondevs/Notion-Skills-for-Claude-28da4445d27180c7af1df7d8615723d0) — Notion 集成
- [Top Agent Skills](https://composio.dev/content/top-claude-skills) — 社区 ranking
